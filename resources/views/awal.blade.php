    ...
    <section id="menu" style="padding: 30px;">
        ...
        <div style="display: flex; justify-content: center; gap: 20px; flex-wrap: wrap;">
            ...
            <div style="background: white; border: 1px solid #ddd; padding: 15px; width: 220px; border-radius: 8px; text-align: center;">
                <div style="height: 120px; background: #eee; margin-bottom: 10px; border-radius: 4px; display: flex; align-items: center; justify-content: center;">
                    {{ $item->nama_menu }}
                </div>
                <h4>{{ $item->nama_menu }}</h4>
                <p style="color: orange; font-weight: bold;">Rp {{ number_format($item->harga, 0, ',', '.') }}</p>
                <a href="/menu/detail" style="display: inline-block; padding: 8px 15px; background: #333; color: white; text-decoration: none; border-radius: 4px; font-size: 14px;">Detail Menu</a>
            </div>
            @endforeach
        </div>
    </section> @endsection ```
